                    ...
                    match function_map.get_mut(&operation) {
                        ...
                        _ => {
                            ...
                            if &operation == "device-telemetry/start" {
                                let dab_request: Result<structs::StartDeviceTelemetryRequest,DabError> =
                                    serde_json::from_str(&payload.clone()).map_err(|e| DabError::Err400(e.to_string()));
                                match dab_request {
                                    Ok(r) => device_telemetry.device_telemetry_start_process(r),
                                    Err(e) => Err(e),
                                }
                            } else if &operation == "device-telemetry/stop" {
                                let dab_request: Result<structs::StopDeviceTelemetryRequest,DabError> =
                                    serde_json::from_str(&payload.clone()).map_err(|e| DabError::Err400(e.to_string()));
                                match dab_request {
